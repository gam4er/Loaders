...
| `--string-obfuscation-strategy <STRATEGY>` | Yes | Optional | Depends on other flags | Concrete value forces one string codec; omitted value chooses one random codec per project; `random` chooses per unique string. |
